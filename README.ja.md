...
- `--summary [LINES]` — 各結果の下に `embed_source` のサマリを表示（デフォルト: 5行）。ファイルを開かずに内容を概観できる
- `--keyword-weight` — ハイブリッド検索におけるキーワードスコアの重み（デフォルト: 0.3）。0にすると純粋なセマンティック検索
- `--include GLOB` — 指定したglobパターンに一致するファイルのみを対象にする（複数指定可、例: `--include "*.c" --include "*.h"`）
- `--exclude GLOB` — 指定したglobパターンに一致するファイルを除外する（複数指定可、例: `--exclude "*/test/*"`）
